...
namespace PreProcessTool
{
	...
	PeBuffer::PeBuffer(const char *fq1Filename, const char *fq2Filename,
		...
		capacity_(capacity), size1_(0), size2_(0), readSize_(0), realReadSize_(0), initReadSize_(
				0), lastIndex1_(0), lastIndex2_(0), lineNum1_(0), lineNum2_(0), seqType_(0), read_pos1(0), read_pos2(0)
	{
		...
		while (index1 < size1_ && readNumTmp1 < initReadSize_)
		{
			if (buf1_[index1] == '\n' || buf1_[index1] == '\0')
			{
				...
				if (lines == 4)000000
				{
                    ...
                    if(filterTile_)
                    {
                        if((tileIsFov_ && isFilterFov(buf1_ + last1 + 1, seqType_)) || isFilterTile(buf1_ + last1 + 1, seqType_)) {
							last1 = pos[3];
							index1++;
							continue;
						}
		...
		while (index2 < size2_ && readNumTmp2 < initReadSize_)
		{
			if (buf2_[index2] == '\n' || buf2_[index2] == '\0')
			{
				...
				if (lines == 4)
				{
					...
					if(filterTile_)
					{
						if ((tileIsFov_ && isFilterFov(buf2_ + last1 + 1, seqType_)) ||
							isFilterTile(buf2_ + last2 + 1, seqType_)) {
							last2 = pos[3];
							index2++;
							continue;
						}
