...
int main_mem(int argc, char *argv[])
{
	...
	gzFile fp, fp2 = 0;
	char *p, *rg_line = 0, *hdr_line = 0, *idxbase2 = 0, *fpath = 0;
	const char *mode = 0;
	void *ko = 0, *ko2 = 0;
	mem_pestat_t pes[4];
	ktp_aux_t aux;
	...
	while ((c = getopt(argc, argv, "51qpaMCSPVYjuk:c:v:s:r:t:R:A:B:O:E:U:w:L:d:T:Q:D:m:I:N:o:f:W:x:G:h:y:K:X:H:")) >= 0) {
		...
		else if (c == 'o' || c == 'f') {xreopen(optarg, "wb", stdout); fpath = optarg;}
	...
	if (optind + 1 >= argc || optind + 4 < argc) {
		fprintf(stderr, "\n");
		fprintf(stderr, "Usage: bwa mem [options] <idxbase1> <idxbase2> <in1.fq> [in2.fq]\n\n");
...
//		fprintf(stderr, "       -s INT        look for internal seeds inside a seed with less than INT occ [%d]\n", opt->split_width);
	...
	char step1[512], step2[512], step3[1024], step4[512];
	char path2[256], path3[256], path4[256];
	sprintf(path2, "%s.umhg19.fq", fpath);
	sprintf(path3, "%s.clean.fastq", path2);
	sprintf(path4, "%s.sam", path2);
	sprintf(step1, "/usr/bgisoft/pmseq200/PMseq-V4.1.1/v4.1.1/nq_step1 0.5 %s %s %s.stat", fpath, path2, fpath);
	sprintf(step2, "/usr/bgisoft/pmseq200/PMseq-V4.1.1/v4.1.1/nq_step2 %s", path2);
	sprintf(step3, "/usr/bgisoft/pmseq200/PMseq-V4.1.1/v4.1.1/bwa mem -k 19 -t 20 -Y -o %s %s %s", path4, idxbase2, path3);

	system(step1);
	system(step2);
	system(step3);

	sprintf(step1, "rm %s", fpath);system(step1);
	char *tail = strstr(fpath, ".sam");
	if (tail) *tail = '\0';
	sprintf(step4, "/usr/bgisoft/pmseq200/PMseq-V4.1.1/v4.1.1/nq_step3 0.8 0.1 %s %s.umhg19.fq.clean.plasmid.fastq", path4, fpath);
	system(step4);

	sprintf(step1, "rm %s", path2);system(step1);
	sprintf(step1, "rm %s", path3);system(step1);
	sprintf(step1, "rm %s", path4);system(step1);
